<?php
	...
	/**
	 * Classe para registro de categoria de produto.
	 * 
	...
	*/
	...
	class Category extends \Database\DatabaseObject{
		...
		/**
		 * Calcula o total de produtos da categoria.
		 * 
		...
		public static function check_url($url){
			...
			$db->query('SELECT id, name FROM '.self::TABLE_NAME.' WHERE slug = "'.$slug.'" AND parent_id IS NULL AND visible = 1');

			if($db->row_count()){
				$category = $db->result(0);
				return array('title' => $category->name, 'subtitle' => '', 'file' => '/site/ecommerce/pages/category/details.php', 'show_title' => false, 'canonical' => true, 'record_id' => $category->id);
			}
